...
func (h hp1) Less(i, j int) bool { return h[i].s < h[j].s }
func (h hp1) Swap(i, j int)      { h[i], h[j] = h[j], h[i] }
func (h *hp1) Push(v any)        { *h = append(*h, v.(tuple)) }
func (h *hp1) Pop() any          { a := *h; v := a[len(a)-1]; *h = a[:len(a)-1]; return v }
