...
func findCrossingTime(n int, k int, time [][]int) int {
	...
	for {
		for len(workInLeft) > 0 {
			if workInLeft[0].x > cur {
				break
			}
			heap.Push(&waitInLeft, heap.Pop(&workInLeft).(pair).y)
		}
		for len(workInRight) > 0 {
			if workInRight[0].x > cur {
				break
			}
			heap.Push(&waitInRight, heap.Pop(&workInRight).(pair).y)
		}
		...
		if !leftToGo && !rightToGo {
			nxt := 1 << 30
			if len(workInLeft) > 0 {
				nxt = common.Min(nxt, workInLeft[0].x)
			}
			if len(workInRight) > 0 {
				nxt = common.Min(nxt, workInRight[0].x)
			}
			cur = nxt
			continue
		}
...
func (h hp3) Less(i, j int) bool  { return h.IntSlice[i] > h.IntSlice[j] }
func (h *hp3) Push(v interface{}) { h.IntSlice = append(h.IntSlice, v.(int)) }
func (h *hp3) Pop() interface{} {
	a := h.IntSlice
	v := a[len(a)-1]
...
func (h hp2) Less(i, j int) bool  { return h[i].x < h[j].x }
