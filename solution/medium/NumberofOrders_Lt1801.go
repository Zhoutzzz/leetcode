	...
	for _, o := range orders {
		...
		if o[2] == 0 {
			for amount > 0 && len(sellOrders) > 0 && sellOrders[0].price <= price {
				...
				amount -= heap.Pop(&sellOrders).(pair1).left
			}
			if amount > 0 {
				heap.Push(&buyOrders, pair1{price, amount})
			}
		} else {
			for amount > 0 && len(buyOrders) > 0 && buyOrders[0].price >= price {
				...
				amount -= heap.Pop(&buyOrders).(pair1).left
			}
			if amount > 0 {
				heap.Push(&sellOrders, pair1{price, amount})
			}
		}
	}
...
type pair1 struct{ price, left int }
type hp []pair1

func (h hp) Len() int            { return len(h) }
func (h hp) Less(i, j int) bool  { return h[i].price > h[j].price }
func (h hp) Swap(i, j int)       { h[i], h[j] = h[j], h[i] }
func (h *hp) Push(v interface{}) { *h = append(*h, v.(pair1)) }
...
func (h *hp2) Push(v interface{}) { *h = append(*h, v.(pair1)) }
